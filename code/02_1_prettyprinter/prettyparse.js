...
class Lexer {
    ...
    readUntil(condition /*: (lexer: Lexer) => boolean */) {
...
function prettyParse(html) {
    ...
    function parseComment() {
        const commentText = lexer.readUntil(lx => lx.match('-->'));
        lexer.consumeMatch('-->');
        return document.createComment(commentText);
    }

    function parseElement() {
        // pretend we are at the position after the opening <
        ...
        //console.log(lexer.readUntil(lx => lx.match(/\/?>/)));
        if (lexer.consumeMatch('>')) {
            if(!voidElements.includes(tagName)) {
                element.appendChild(parseContent());
                lexer.consumeMatch('</');
                lexer.readUntil(lx => lx.consumeMatch('>'));
            }
        } else {
            // no nested content
            lexer.consumeMatch('/>');
    ...
    function parseAttribute() {
        ...
        if (lexer.consumeMatch(/\s*=\s*/)) {
            let value;
            if (lexer.match(/["']/)) {
                const closingChar = lexer.read(); // read the enclosing character " or '
                value = lexer.readUntil(lx => lx.match(closingChar));
                lexer.consumeMatch(closingChar);
            } else {
                value = lexer.readUntil(lx => lx.match(/[\/\s>]/));
            }
    ...
    switch (node.nodeType) {
        ...
        case Node.ELEMENT_NODE: {
            ...
            if(voidElements.includes(node.nodeName.toLowerCase())) {
                const voidElement = document.createElement('pp-voidelement');
                voidElement.innerText = node.nodeName.toLowerCase();
                return voidElement;
            }
...
function testLexer(html) {
    ...
    let comment = lexer.readUntil(aLexer => aLexer.match('-->'));
